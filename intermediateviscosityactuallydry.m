...
h0 = 1/(1+w); % setting the initial condition for the ODEs
...
% ODE for dry foam, V_s = 0.3
[t,h] = ode45(@(t,h) -(1-h)^(-1/4)/(1/(0.3*h^(3/2))+3/(10*h^(5/2)*(1+w))), tspan, h0);
graph3 = loglog(t,h,'--') % log-log plot, dashed line
set(graph3,'LineWidth',2)

% ODE for dry foam, V_s = 1
[t,h] = ode45(@(t,h) -(1-h)^(-1/4)/(1/(h^(3/2))+3/(10*h^(5/2)*(1+w))), tspan, h0);
graph4 = loglog(t,h,'--') % log-log plot, dashed line
set(graph4,'LineWidth',2)

% ODE for dry foam, V_s = 5
[t,h] = ode45(@(t,h) -(1-h)^(-1/4)/(1/(5*h^(3/2))+3/(10*h^(5/2)*(1+w))), tspan, h0);
graph5 = loglog(t,h,'--') % log-log plot, dashed line
set(graph5,'LineWidth',2)

title('\textbf{Dry foam: Log-log plot of $\tilde{h}$ under all regimes}','Interpreter', 'latex','fontsize',16)
